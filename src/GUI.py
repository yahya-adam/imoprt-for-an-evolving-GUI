...
class TransformManager:
    ...
    def load_plugins(self):
        print("\n=== Plugin Loading ===")
        plugin_dir = "plugins"
        script_dir = os.path.dirname(os.path.abspath(__file__))  # Directory of GUI (2).py
...
class App:
    ...
    # Main widgets
        self.import_btn = CTkButton(root, text="Import Dataset", command=self.import_dataset) 
        self.import_btn.pack(fill= "none", expand= None,padx=5 ,pady=10, anchor= "sw")

        self.script_btn = CTkButton(root, text="Run Script", command=self.run_script)
        self.script_btn.pack(fill= None, expand= None,padx=5 ,pady=10, anchor= "sw")

        self.table_selector = ttk.Combobox(root, textvariable=self.table_var, state="readonly")
        ...
        self.column_list = Listbox(root, width=100, height=7, selectmode=MULTIPLE)
        self.column_list.pack(fill="none", expand= None, padx=5, ipadx= 10, anchor= "w")

        self.merge_btn = CTkButton(root, text="Merge Columns", command=self.merge_columns)
    ...
    def load_config(self, filename):
        try:
            ...
            if not os.path.exists(config_path):
                raise FileNotFoundError(f"'{filename}' not found in directory: {script_dir}")
    ...
    def import_dataset(self):
        ...
        # Handle table conflicts
            ...
            # Update DataFrame to use sanitized column names
